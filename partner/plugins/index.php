...
foreach($all_plugins as $key=>$value) {
    ...
    if((bool)!$latest_version) {
        echo '<button type="button" data-plugin-id="'.$plugins->getSettingsValues($key,"plugin_id").'" data-plugin-key="'.$i.'" data-plugin-name="'.$key.'"'; if(!file_exists(BASEDIR . "plugins/".$key)) { echo "style='display:none'"; } echo ' class="btn btn-sm btn-warning me-2 UpdateModal">'.$lang["PARTNER"]["PLUGINS"]["UPDATE"].'</button>';
    }
    echo
