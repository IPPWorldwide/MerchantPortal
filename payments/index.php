...
<div class="row row-cols-md-2 mb-2">
    <div class="col-6">
        ...
        if(($transaction_data->method === "CAPTURE") || ($transaction_data->method === "SALE")) {
            if(!isset($IPP_CONFIG["PORTAL_LOCAL_DEACTIVATE_REFUNDS"]) || ($IPP_CONFIG["PORTAL_LOCAL_DEACTIVATE_REFUNDS"] !== "1")) {
                ...
                <div class="row row-cols-md-2 mb-2">
                    ...
                    <div class="col themed-grid-col"><button data-type="Refund"'; if($merchant_data->limitations->REFUND->active == '1'){ echo 'disabled'; };  echo ' class="btnRefund btnAction btn btn-warning">'.$lang["COMPANY"]["PAYMENT"]["REFUND"].'</button> '; if($merchant_data->limitations->REFUND->active == '1'){ echo '<br><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
                        <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                        <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/>
                      </svg> This function have been disabled. Please contact support'; };  echo '</div>
                </div>        
        ...
        if(!isset($IPP_CONFIG["PORTAL_LOCAL_DEACTIVATE_VOID"]) || ($IPP_CONFIG["PORTAL_LOCAL_DEACTIVATE_VOID"] !== "1")) {
            ...
            <div class="col themed-grid-col"><button data-type="Void"'; if($merchant_data->limitations->VOID->active == '1'){ echo 'disabled'; };  echo ' class="btnAction btnVoid btn btn-warning '; if($merchant_data->limitations->VOID->active == '1'){ echo 'disable'; };  echo '">'.$lang["COMPANY"]["PAYMENT"]["VOID"].'</button> '; if($merchant_data->limitations->VOID->active == '1'){ echo '<br><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
                <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
                <path d="m8.93 6.588-2.29.287-.082.38.45.083c.294.07.352.176.288.469l-.738 3.468c-.194.897.105 1.319.808 1.319.545 0 1.178-.252 1.465-.598l.088-.416c-.2.176-.492.246-.686.246-.275 0-.375-.193-.304-.533L8.93 6.588zM9 4.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"/>
              </svg> This function have been disabled. Please contact support'; };  echo '</div>
        </div>';
